...
function ForgotPassword() {
    ...
    const handleForgotPassword = async (e) => {
        ...
        try {
            ...
            setLoading(false)
            toast.success(result.data?.message || "Reset instructions sent")

            if (result.data?.resetUrl) {
                const token = String(result.data.resetUrl).split("/").pop()
                if (token) {
    ...
    return (
        <div className='w-[100vw] h-[100vh] flex items-center justify-center relative'>
            <div className='w-[50px] h-[50px] bg-[red] cursor-pointer absolute top-[10%] left-[20px] rounded-[50%] flex items-center justify-center' onClick={()=>navigate("/login")}><FaArrowLeftLong className='w-[25px] h-[25px] text-[white]' /></div>
            <form className='max-w-[900px] w-[90%] h-[600px] flex items-center justify-center flex-col md:items-start gap-[12px]' onSubmit={handleForgotPassword}>
                ...
                <p className='text-[16px] text-[#555]'>Enter your account email to reset password.</p>
                ...
                <button className='px-[50px] py-[10px] bg-[red] text-[white] text-[18px] md:px-[100px] rounded-lg mt-[20px]' disabled={loading}>{loading ? "Loading..." : "Send Reset Link"}</button>
            </form>
        </div>
    )
}
